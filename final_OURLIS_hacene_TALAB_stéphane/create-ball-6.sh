...
#on fait ça qu'une seule fois .
if [ ! -f my-ball.sh ]; then
...
#pour ne pas faire tout ça pour rien ;
	if [ -e $i ];then
		# Pour récupérer l'octet des droits d'accès du propriétaire :
		# le tail -n +2 je l'ai fait pour retirer l'indicateur de dossiers ou fichiers ( càd soit "d" soit "-"  soit lien symbloique) ou bien lien symbolique .
		#le grep c'est juste pour supprimer la ligne vide de trop à la fin .
		ls -ld "$i" | cut -d ' ' -f 1 | sed -E 's/(.{1})/\1\n/g' | grep -E -v "^ *$" | tail -n +2 | while read line
		do
		    if [ "$line" = "-" ]; then
			echo "0"
		    elif [ "$line" = "r" ]; then
		...
		done > on_test_sed.txt
		#récupération des 3 premiers lignes qui sont en faite les droits du propriétaire
		d_prop=$(expr `head -n 3 on_test_sed.txt | tr '\n' '+' | sed -E 's/\+/ \+ /g'`0)
	#récupération des 3lignes suivantes qui sont en faite les droits du groupe
		d_groupe=$(expr `tail -n +4 on_test_sed.txt | head -n 3 | tr '\n' '+' | sed -E 's/\+/ \+ /g'`0)
	#récupération des 3lignes suivantes qui sont en faite les droits des autres
		d_others=$(expr `tail -n 3 on_test_sed.txt | tr '\n' '+' | sed -E 's/\+/ \+ /g'`0)

		# les droits d'accées donc :
		D_ACCES=$(expr "$d_prop" \* 100 + "$d_groupe" \* 10 + "$d_others")
	fi
 	if [ -f $i ]; then
#ici on doit utiliser la double " >> " car on veut pas ecraser ce qui est déja mis dans my-ball.sh
#et on va utiliser TAG_script AVEC EXPENSION DE VARIABLE ; ca on voudrait bien vérifier si le fichier à reconstruire a (verbe avoir) ses parents .
    cat << TAG_script_files >> my-ball.sh
...
"./decodeur_partie3" "-uu" << 'TAGDEFIN' > "$i"
$("./my_sae_encodage" "-uu" "$i" | sed 's/ *$//g')
TAGDEFIN
chmod $D_ACCES $i
TAG_script_files

	elif [ -d $i ];then
     cat << TAG_script_directories >> my-ball.sh
     	mkdir $i
     	chmod $D_ACCES $i
TAG_script_directories
		#après beaucoup d'erreur concernant le dossier vide dans l'arboressance ; on verifie si le retour de ls $i n'est pas vide ; si oui on appele le script encode si non ; on sort .
		#l'option ( -n ) trouver dans flashcard du module systéme d'exploitation ; vérifie si un string n'est pas vide ;;; 
		if [ -n "$(ls "$i")" ];then
			# Le répertoire n'est pas vide, appeler le script récursivement
			$0 $i/*
		fi
	else
    		echo "Le fichier '$i' n'existe pas ou bien ce n\'est pas un fichier régulier"
  	fi
done
for i in $*
do
